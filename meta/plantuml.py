...
class PlantUmlNode():
    ...
    def write(self, f):
        ...
        for item in self.attributes:
            f.write('<{0}> {1}: {2}\n'.format(item['type'], item['name'], item['contract']))
        f.write('-- Workflows --\n')
        ...
        if not self.is_virtual:
            ...
            if 'Properties' in self.manifest.keys():
                for item in self.manifest['Properties']:
                    ...
                    uml_class.add_attribute(item, type=item_type, contract=item_contract)
...
class DslPlantUmlGraph():
    ...
    def write(self, classname, level=0):
        ...
        for ext_class in ext_classes:
            self.add_edge(from_node=ext_class, to_node=classname, edge_type='<..')
        ...
        if node.parent_dn:
            self.add_edge(from_node=node.parent_dn, to_node=classname, edge_type='<|--')
            self.write(node.parent_dn, level + 1)

        if level == 0:
            for edge in self._edges:
                self._file.write('{0} {2} {1}\n'.format(edge['from_node'], edge['to_node'], edge['type']))
